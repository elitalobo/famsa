...
def experiment_main(path, path1, pathshap, pathshap1, ycol, columns,xmin,xmax):
	...
	* We print out the rate at which features occur in the top three features
	...
	adv_lime = Adversarial_Lime_Model1(xtrain, xtest, path,  columns, ss, xmin,xmax,racist_model_f(), innocuous_model_psi()).train(xtrain, ytrain, categorical_features=[features.index('unrelated_column_one'),features.index('unrelated_column_two')], feature_names=features, perturbation_multiplier=30)
	...
	adv_lime = Adversarial_Lime_Model1(xtrain, xtest,path1,  columns, ss, xmin,xmax,racist_model_f(), innocuous_model_psi_two()).train(xtrain, ytrain, feature_names=features, perturbation_multiplier=30, categorical_features=[features.index('unrelated_column_one'),features.index('unrelated_column_two')])
	adv_explainer = lime.lime_tabular.LimeTabularExplainer(xtrain, feature_names=adv_lime.get_column_names(), discretize_continuous=False, categorical_features=[features.index('unrelated_column_one'),features.index('unrelated_column_two')])
	...
	adv_shap = Adversarial_Kernel_SHAP_Model(xtrain, xtest, pathshap, columns,racist_model_f(), innocuous_model_psi()).train(xtrain, ytrain, feature_names=features,xgb_estimators=30)
	...
	background_distribution = shap.kmeans(xtrain,10)
	adv_shap = Adversarial_Kernel_SHAP_Model(xtrain, xtest, pathshap1, columns,racist_model_f(), innocuous_model_psi_two()).train(xtrain, ytrain, feature_names=features,xgb_estimators=30)
	adv_kerenel_explainer = shap.KernelExplainer(adv_shap.predict, background_distribution)
	explanations = adv_kerenel_explainer.shap_values(xtest)
...
if __name__ == "__main__":
	path = "../../bench/fairml/cc_ood/"
	path1 = "../../bench/fairml/cc_ood1/"
	pathshap = "../../bench/fairml/cc_shapood/"
	pathshap1 = "../../bench/fairml/cc_shapood1/"
