...
def lime_call(xgb, sample = None, nb_samples = 5, feats='all',
        nb_features_in_exp=5,writer=None, index=None):

    timer = resource.getrusage(resource.RUSAGE_CHILDREN).ru_utime + \
            resource.getrusage(resource.RUSAGE_SELF).ru_utime
    num_features = xgb.X_train.shape[1]
    nb_features_in_exp = num_features
    ...
    if (sample is not None):
        ...
        for i in range(xgb.num_class):
            ...
            res.append(s_human_readable)
        timer = resource.getrusage(resource.RUSAGE_CHILDREN).ru_utime + \
                resource.getrusage(resource.RUSAGE_SELF).ru_utime - timer
        print('  time: {0:.2f}'.format(timer))

        lent = len(f2imap.keys())
        features_ = [None for x in range(lent)]
        for key, value in f2imap.items():
            features_[value]=key

        f_str = ":".join(features_)
        scores =  list(exp.local_exp.values())[0]
        expl_scores=np.zeros(len(scores))

        for score in scores:
            expl_scores[int(score[0])]=score[1]
        expl_str = ":".join([str(x) for x in expl_scores])
        res_str = str(index) +","+  str(y_pred) + "," + expl_str +","+ f_str +"," + ":".join(res) +"\n"
