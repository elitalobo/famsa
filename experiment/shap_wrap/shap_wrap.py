...
#
#==============================================================================
def shap_call(xgb, sample = None, feats='all', nb_features_in_exp = None, writer=None, index=None):
    ...
    if (sample is not None):
        ...
        features_ = [None for idx in range(lent)]
        ...
        expl_str = ":".join([str(e) for e in expl])
        values_str = [str(np.round(x, 3)) for x in sum_values]
        res = str(index) + "," +  str(y_pred) + "," + ":".join(values_str)
        feat_str = ":".join(features_)
        res += "," + feat_str + "," + expl_str+ "\n"
